...
Sidecar fusion plan ABI, 2026-06-30 14:35 AKDT:
...
- Diff hygiene:
  `git diff --check`
  - Passed.
- Live HTTPS sidecar fusion-plan benchmark:
  ...
  - Passed the performance gate with suite status `complete`, suite gate
    `pass`, scenario `good`, `residentStageMs=4.9`,
    `residentStageStepsPerSecond=204.08`, preflight fallback
    ...
    `sidecarFusionStageCount=2`, compact summary `mapAsyncWaitMs=null`, and
...
Thermal sidecar-aware sequence evidence, 2026-06-30 16:36 AKDT:
...
- VPN server availability:
  `curl -k -I --max-time 10 https://100.86.83.35:5173/` and
  `curl -I --max-time 10 http://100.86.83.35:5174/`
  - Passed: HTTPS returned HTTP/2 200, HTTP returned HTTP/1.1 200, and
    `ss -ltnp` showed both listeners on `0.0.0.0`.
- Live HTTPS sidecar-aware sequence benchmark:
  ...
  - Passed the performance gate with suite status `complete`, suite gate
    ...
    `sidecarOnlyBlocked=true`,
    `sidecarFusionStepEvidenceStatus=sidecar-fusion-step-evidence-ready`,
    sidecar executed/passed stages `2/2`,
    ...
    sidecar-aware completed/passed steps `2/2`, `allStepsPassed=true`,
    `residentStageMs=9.6`, `residentStageStepsPerSecond=104.17`, compact
    summary `mapAsyncWaitMs=null`, and compact readback byte length `0`.
  - Caveat: the nested two-step direct probe status remained `bad` with issues
...
Thermal sidecar direct runner contract, 2026-06-30 17:31 AKDT:
...
- VPN server availability:
  ...
  - Passed: HTTPS returned HTTP/2 200, HTTP returned HTTP/1.1 200, and
    `ss -ltnp` showed both listeners on `0.0.0.0`.
- Live HTTPS direct-runner contract benchmark:
  ...
  - Passed the performance gate with suite status `complete`, suite gate
    ...
    `residentStageStepsPerSecond=163.93`, compact summary
    `mapAsyncWaitMs=null`, and compact readback byte length `0`.
  - Caveat: the nested two-step direct probe status remained `bad` with issues
    `missing-max-speed` and `no-positive-displacement`; this test records
    direct-runner readiness, not visible motion quality.

Thermal sidecar direct runner selected, 2026-06-30 17:45 AKDT:
...
- Benchmark/probe source harness:
  ...
  - Passed: `11/11`.
- VPN server availability:
  `curl -k -I https://100.86.83.35:5173/` and
  `curl -I http://100.86.83.35:5174/`
  - Passed: HTTPS returned HTTP/2 200, HTTP returned HTTP/1.1 200, and
    `ss -ltnp` showed both listeners on `0.0.0.0`.
- Live HTTPS direct-runner benchmark:
  ...
  - Passed the performance gate with suite status `complete`, suite gate
    ...
    `residentStageStepsPerSecond=212.77`, compact summary
    `mapAsyncWaitMs=null`, and compact readback byte length `0`.
  - Caveat: the nested two-step direct probe status remained `bad` with issues
    `missing-max-speed` and `no-positive-displacement`; this test records

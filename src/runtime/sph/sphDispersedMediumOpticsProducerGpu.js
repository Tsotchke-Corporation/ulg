...
function canonicalProducerAdoptionDeclaration({
  ...
  return Object.freeze({
    schema: ULG_SPH_DISPERSED_MEDIUM_OPTICS_SCHEMA,
    status: summary.readyRowCount > 0
    ...
    readyOpticalStateRouteCount: summary.readyOpticalStateIds.length,
    rowLayout: SPH_DISPERSED_MEDIUM_OPTICS_ROW_LAYOUT,
...
export function deriveSphDispersedMediumOpticsProducerReference({
  ...
  for (let particleIndex = 0;
    ...
    if (sourceRows[opticsOffset + 3] !== SPH_DISPERSED_MEDIUM_OPTICS_STATUS.ready) {
      rows.fill(0, opticsOffset + 4, opticsOffset + 8);
      continue;
    }
    const route = closureRowForOpticsPrefix(opticalClosureTable, {
      dispersedMaterialId: sourceRows[opticsOffset],
      dispersedPhaseId: sourceRows[opticsOffset + 1],
      opticalStateId: sourceRows[opticsOffset + 2]
    });
    if (!route) {
      rows.fill(0, opticsOffset + 4, opticsOffset + 8);
    ...
    const priorMassKg = Math.fround(sourceRows[opticsOffset + 4]);
    ...
    if (
      ...
      || moments.dispersedMaterialId !== sourceRows[opticsOffset]
      || moments.dispersedPhaseId !== sourceRows[opticsOffset + 1]
      || moments.opticalStateId !== sourceRows[opticsOffset + 2]
    ) {
      throw new RangeError(
  ...
  return Object.freeze({
    ...
    immutableDeclarationLanes: Object.freeze([0, 1, 2, 3]),
...
fn optical_moments(route: ClosureRoute, mass_kg: f32) -> vec4<f32> {
  ...
  let q_scattering = route.optics0.y;
  let q_absorption = route.optics0.z;
  let asymmetry_factor = route.optics0.w;
  var geometric_cross_section_m2 = 0.0;
  if (morphology == COMPACT_MORPHOLOGY) {
    let radius_m = pow(mass_kg / ((4.0 * PI / 3.0) * density), 1.0 / 3.0);
...
export function sphDispersedMediumOpticsProducerPipelineDescriptors() {
  ...
  return Object.freeze([
    Object.freeze({
      cacheKey: 'ulg-sph-dispersed-medium-optics-producer.v0.preflight',
    ...
    Object.freeze({
      cacheKey: 'ulg-sph-dispersed-medium-optics-producer.v0.apply',
...
export function createSphDispersedMediumOpticsProducerWebGpuEncoderStage({
  ...
  try {
    ...
    const [preflightDescriptor, applyDescriptor] =
    ...
    const entries = [
      ...
      {
        binding: 4,
        resource: { buffer: resolvedPriorOpticsBuffer, size: opticsBufferByteLength }
      },
      {
        binding: 5,

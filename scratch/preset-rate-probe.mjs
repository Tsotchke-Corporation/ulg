...
const page = await browser.newPage({ ignoreHTTPSErrors: true, viewport: { width: Number(process.env.ULG_PROBE_W || 1600), height: Number(process.env.ULG_PROBE_H || 900) } });
...
for (let i = 0; i < 140; i += 1) {
  ...
  if (samples[n - 1].steps >= Number(process.env.ULG_PROBE_BREAK_STEPS || 192)) break;

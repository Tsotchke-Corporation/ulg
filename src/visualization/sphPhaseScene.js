...
export function resolveResidentExtensionSurfaceRendererCapability({
  ...
  if (nativeSurfaceConsumerSupported) {
    ...
    reason = 'native WebGPU surface consumer is wired as the next no-overlay path but must pass browser console and pixel validation before promotion';
...
export function resolveResidentSurfaceVisibleGpuConsumer({
  ...
  if (inputReady && inputKind === 'render-field-buffers') {
    ...
    reason = nativeWebGpuBridgeBound
      ? 'engine-owned native WebGPU surface consumer must pass pixel validation or the resident-device readback fallback before it can count as visible no-readback rendering'
...
export function createSphPhaseScene(container, {
  ...
  function createSphResidentRenderRowsThreePointBridge({
    ...
    if (useSphereBridge) {
      ...
      for (const entry of groupsBySurface.values()) {
        ...
        const prepareSphereMaterial = (sourceMaterial) => {
          if (!sourceMaterial) return sourceMaterial;
          sourceMaterial.userData.renderRowSphereMaterialSignature = nextMaterialSignature;
          stabilizeRenderRowSphereBridgeMaterial(sourceMaterial, {
            descriptor,
            fallbackColorSrgb: bridgeFallbackColorSrgb
          });
          if (!useThreeWebGpuResidentBridgeMaterialProxy) return sourceMaterial;
          if (
            sourceMaterial.userData.surfaceMaterialRendererProxyReason
            === 'three-webgpu-render-row-spheres-basic-material-pipeline-proxy'
          ) {
            sphereBridgeMaterialRendererProxyCount += 1;
            return sourceMaterial;
          }
          const proxyMaterial = createThreeWebGpuResidentBridgeMaterialProxy(sourceMaterial, {
            descriptor,
            fallbackColorSrgb: bridgeFallbackColorSrgb,
            bridgeMode: SPH_THREE_RENDER_ROW_SPHERES_BRIDGE_MODE,
            proxyReason: 'three-webgpu-render-row-spheres-basic-material-pipeline-proxy'
          });
          proxyMaterial.userData.renderRowSphereMaterialSignature = nextMaterialSignature;
          if (proxyMaterial.userData.surfaceMaterialRendererProxy) {
            sphereBridgeMaterialRendererProxyCount += 1;
          }
        ...
        if (!canReuseMesh) {
          ...
          const sphereGeometry = useThreeWebGpuResidentBridgeMaterialProxy
            && typeof Three.OctahedronGeometry === 'function'
            ? new Three.OctahedronGeometry(1, 0)
            : new Three.SphereGeometry(1, 8, 6);
          if (useThreeWebGpuResidentBridgeMaterialProxy) {
            sphereGeometry.userData.threeWebGpuMappedBufferSafeProxy = true;
            sphereBridgeGeometryProxyCount += 1;
          }
          const sourceMaterial = makeSurfaceMaterial(descriptor, properties, nextOptics);
          const material = prepareSphereMaterial(sourceMaterial);
        ...
        if (previousMaterialSignature !== nextMaterialSignature) {
          const sourceReplacementMaterial = makeSurfaceMaterial(descriptor, properties, nextOptics);
          const replacementMaterial = prepareSphereMaterial(sourceReplacementMaterial);
          const oldMaterial = mesh.material;
          mesh.material = replacementMaterial;
          disposeSphereMaterial(oldMaterial);
          if (replacementMaterial !== sourceReplacementMaterial) sourceReplacementMaterial.dispose?.();
          material = replacementMaterial;
        } else {
          const preparedMaterial = prepareSphereMaterial(material);
          if (preparedMaterial !== material) {
            const oldMaterial = mesh.material;
    ...
    Object.assign(bridge, {
      ...
      opticalRenderSource: useSphereBridge
        ? (useThreeWebGpuResidentBridgeMaterialProxy
          ? 'render-row-material-webgpu-basic-proxy'
  ...
  function createSphResidentSurfaceDrawThreeWebGpuBufferBridge({
    surfaceDrawExecution,
    materialProperties = currentMaterialProperties,
    rendererCapability = null
  } = {}) {
    const schema = 'peercompute.ulg.sph-resident-surface-draw-render-bridge.v0';
    const rendererBridge = SPH_THREE_WEBGPU_SURFACE_BUFFER_BRIDGE_MODE;
    const capability = rendererCapability || resolveResidentExtensionSurfaceRendererCapability({
      renderer,
      renderBridgeMode: rendererBridge,
      readbackMode: RESIDENT_NO_FULL_READBACK_MODE
    });
    if (!capability.visibleNoReadbackSupported) {
      return {
        schema,
        status: 'three-webgpu-surface-buffers-unavailable',
        reason: capability.reason || 'Three WebGPU same-device buffer geometry is unavailable',
        rendererBridge,
    ...
    const sourceVertexRowCount = Math.max(
      ...
      Math.floor(Number(
        surfaceDrawExecution?.sourceVertexRowCount
          ?? surfaceDrawExecution?.vertexCount
          ?? (vertexRowsBufferByteLength / (rowStride * Float32Array.BYTES_PER_ELEMENT))
      ) || 0)
    );
    ...
    const positionBinding = createThreeWebGpuExternalInterleavedBufferAttribute({
      renderer,
      buffer: vertexRowsBuffer,
      count: sourceVertexRowCount,
      stride: rowStride,
      itemSize: 3,
      offset: SPH_SURFACE_VERTEX_ROW_INDEX.positionXM,
      name: 'position'
    });
    const useExternalNormalAttribute = false;
    const normalBinding = useExternalNormalAttribute ? {
      attribute: new Three.InterleavedBufferAttribute(
        positionBinding.interleavedBuffer,
        3,
        SPH_SURFACE_VERTEX_ROW_INDEX.normalX,
        false
      )
    } : null;
    if (normalBinding) normalBinding.attribute.name = 'normal';

    const drawRecords = resolveThreeWebGpuSurfaceBufferDrawRecords({
      surfaceDrawExecution,
      rowStride
    });
    const aggregateIndirectBuffer = surfaceDrawExecution?.drawAggregateIndirectRowsBuffer || null;
    const aggregateIndirectBinding = (
      SPH_THREE_WEBGPU_SURFACE_BUFFER_INDIRECT_RUNTIME_VALIDATED
      &&
      drawRecords.conservativeNoReadbackDrawRange
      && surfaceDrawExecution?.drawAggregateIndirectRowsBufferRetained
      && surfaceDrawExecution?.surfaceDrawGpuOnlyAggregateIndirectReady
      && aggregateIndirectBuffer
    )
      ? createThreeWebGpuExternalIndirectBufferAttribute({
        renderer,
        buffer: aggregateIndirectBuffer,
        itemSize: SPH_GPU_RENDER_SURFACE_DRAW_INDIRECT_UINTS,
        name: 'ulg-sph-surface-draw-aggregate-indirect'
      })
      : null;
    const sourceSurfaces = drawRecords.records;
    const materialMap = buildSphRenderMaterialMap(materialProperties || {}, sphReactionTable);
    const materialRenderPolicy = resolveSceneSurfaceMaterialRenderPolicy();
    const group = new Three.Group();
    ...
    for (let recordIndex = 0; recordIndex < sourceSurfaces.length; recordIndex += 1) {
      const sourceSurface = sourceSurfaces[recordIndex] || {};
      const vertexOffset = Math.max(0, Math.floor(Number(sourceSurface.vertexOffset) || 0));
      const requestedVertexCount = Math.max(0, Math.floor(Number(sourceSurface.vertexCount) || 0));
      const vertexCount = Math.min(sourceVertexRowCount - vertexOffset, requestedVertexCount);
      const alignedVertexCount = vertexCount - (vertexCount % 3);
      if (alignedVertexCount < 3) continue;

      const descriptor = renderDescriptorForSurfaceRecord(sourceSurface, recordIndex, {
        materialProperties,
        reactionTable: sphReactionTable,
        materialMap
      });
      ...
      if (normalBinding) geometry.setAttribute('normal', normalBinding.attribute);
      if (aggregateIndirectBinding && drawRecords.conservativeNoReadbackDrawRange) {
        geometry.setIndirect(aggregateIndirectBinding.attribute, 0);
      }
      ...
      let material = makeSurfaceMaterial(descriptor, properties);
      stabilizeSurfaceMeshMaterialForRenderer(material, {
        descriptor,
        rendererMaterialPolicy: materialRenderPolicy,
        bridgeMode: rendererBridge
      });
      material = createThreeWebGpuSurfaceBufferMaterialProxy(material, {
        descriptor
      });
      if (material.userData.surfaceMaterialRendererProxy) surfaceMaterialRendererProxyCount += 1;
      ...
      mesh.userData.phaseId = Number.isFinite(Number(sourceSurface.phaseId))
        ? Math.round(Number(sourceSurface.phaseId))
        : null;
      mesh.userData.surfaceIndex = Math.max(0, Math.round(Number(sourceSurface.surfaceIndex ?? recordIndex) || 0));
      mesh.userData.surfaceDrawVertexOffset = vertexOffset;
      mesh.userData.surfaceDrawVertexCount = alignedVertexCount;
      mesh.userData.surfaceDrawTriangleCount = alignedVertexCount / 3;
      mesh.userData.surfaceDrawIndirect = Boolean(aggregateIndirectBinding && drawRecords.conservativeNoReadbackDrawRange);
      mesh.userData.surfaceDrawSource = surfaceDrawExecution.schema ?? null;
    ...
    if (meshes.length > 0) {
      scene.add(group);
      suppressThreeSurfaceMeshesForResidentOverlay('resident-surface-draw-three-webgpu-buffers-active');
    }
    const status = meshes.length > 0
      ? SPH_THREE_WEBGPU_SURFACE_BUFFER_BRIDGE_STATUS
      : 'three-webgpu-surface-buffers-empty';
    const bridge = {
      schema,
      status,
      rendererBridge,
      visibleRenderSource: meshes.length > 0
        ? 'resident-surface-draw-three-webgpu-external-buffer'
        : 'resident-surface-draw-three-webgpu-external-buffer-empty',
      reason: meshes.length > 0 ? null : 'retained surface draw metadata had no active triangle rows',
      overlayPolicy: resolveSceneResidentSurfaceDrawOverlayPolicy(),
      threeSurfaceGroup: group,
      threeMeshes: meshes,
      threeMeshCount: meshes.length,
      threeGeometryByteLength: 0,
      externalGpuBufferGeometry: true,
      externalGpuBufferByteLength: vertexRowsBufferByteLength,
      externalGpuBufferVertexRowStrideFloats: rowStride,
      externalGpuBufferNormalAttribute: Boolean(normalBinding),
      externalGpuBufferNormalAttributeDisabledReason: normalBinding
        ? null
        : 'Three WebGPU surface-buffer diagnostic uses unlit basic material and omits external normals until pipeline validation passes',
      externalGpuBufferIndirect: Boolean(aggregateIndirectBinding),
      externalGpuBufferIndirectRuntimeValidated: SPH_THREE_WEBGPU_SURFACE_BUFFER_INDIRECT_RUNTIME_VALIDATED,
      externalGpuBufferIndirectDisabledReason: SPH_THREE_WEBGPU_SURFACE_BUFFER_INDIRECT_RUNTIME_VALIDATED
        ? null
        : 'Three WebGPU indirect storage draw binding is disabled until browser console and pixel validation pass',
      externalGpuBufferIndirectByteLength: aggregateIndirectBinding
        ? (surfaceDrawExecution?.drawAggregateIndirectRowsBufferByteLength ?? 0)
        : 0,
      conservativeNoReadbackDrawRange: Boolean(drawRecords.conservativeNoReadbackDrawRange),
      conservativeNoReadbackDrawRangeUsesIndirect: Boolean(
        drawRecords.conservativeNoReadbackDrawRange
        && aggregateIndirectBinding
      ),
      drawRecordsStatus: drawRecords.status,
      drawRecordsReason: drawRecords.reason,
      aggregateVertexCount: drawRecords.aggregateVertexCount,
      frameCount: 0,
      lastRenderStatus: meshes.length > 0
        ? 'three-webgpu-surface-buffers-submitted'
        : 'three-webgpu-surface-buffers-empty',
      drawOrderingPolicy: drawRecords.conservativeNoReadbackDrawRange
        ? 'three-webgpu-external-buffer-conservative-aggregate-draw-range'
        : 'three-webgpu-external-buffer-scene-render-order',
      drawOrderCount: meshes.length,
      drawOrderSurfaceIndices: meshes.map((mesh) => mesh.userData.surfaceIndex),
      drawOrderIndirectOffsets: [],
      depthPolicy: 'three-webgpu-managed-depth-buffer',
      depthAttachmentFormat: null,
      depthAttachmentReady: true,
      transparencyCompositeMode: 'three-webgpu-material-depth-sort',
      oitAccumFormat: null,
      oitRevealFormat: null,
      oitTargetsReady: false,
      lastOpaqueDrawCount: opaqueDrawCount,
      lastTransparentDrawCount: transparentDrawCount,
      opticalRenderSource: 'closure-derived-three-materials',
      opticalRecordCount: opticalGpuTable?.recordCount ?? 0,
      opticalRecordStrideFloats: opticalGpuTable?.recordStrideFloats ?? 0,
      opticalSpectralSampleCount: opticalGpuTable?.spectralSampleCount ?? 0,
  ...
  function hasCpuSurfaceGeometryBatches(batches = []) {
    return Array.isArray(batches) && batches.some((batch) => {
      const count = Math.max(0, Math.round(Number(batch?.count) || 0));
      return count > 0
        && Math.floor((batch?.normalizedPositions?.length || 0) / 3) >= count
        && Math.floor((batch?.colorsRgb?.length || 0) / 3) >= count;
    });
  }

  function averageBatchColor(batch) {
    if (Array.isArray(batch?.averageColorRgb) || ArrayBuffer.isView(batch?.averageColorRgb)) {
      const color = [
        Number(batch.averageColorRgb[0]),
        Number(batch.averageColorRgb[1]),
        Number(batch.averageColorRgb[2])
      ];
      return [
        clamp(Number.isFinite(color[0]) ? color[0] : 1, 0, 1),
        clamp(Number.isFinite(color[1]) ? color[1] : 1, 0, 1),
        clamp(Number.isFinite(color[2]) ? color[2] : 1, 0, 1)
      ];
    }
    if (!batch?.colorsRgb?.length || !batch.count) return [1, 1, 1];
    const sampleCount = Math.min(batch.count, Math.floor(batch.colorsRgb.length / 3));
    ...
    return [r / sampleCount, g / sampleCount, b / sampleCount].map((value) => clamp(value, 0, 1));
  }
  ...
  async function extractNativeMarchingCubesSurfaceForRenderFieldDescriptor({
    ...
    try {
      ...
      const useNativeWebGpuSurfaceConsumerBridge =
        renderBridgeMode === SPH_NATIVE_WEBGPU_SURFACE_CONSUMER_BRIDGE_MODE;
      ...
      const renderFieldSurfaceCount = Math.max(0, Math.round(Number(renderFieldExecution.surfaceCount) || 0));
      const renderFieldTotalCells = Math.max(0, Math.round(Number(renderFieldExecution.totalFieldCells) || 0));
      if (renderFieldSurfaceCount === 0 || renderFieldTotalCells === 0) {
        const unavailable = residentSurfaceDrawUnavailable(
          'retained WebGPU render-field has no surface cells; surface draw dispatch skipped',
          { renderFieldExecution, overlayPolicy: resolveSceneResidentSurfaceDrawOverlayPolicy() }
        );
        unavailable.status = 'resident-surface-draw-empty-render-field';
        unavailable.renderFieldBufferMode = 'released-empty-render-field';
        ...
        unavailable.visibleRendererBridge = useThreeWebGpuSurfaceBufferBridge
          ...
          : (useThreeCompactVertexBridge
            ...
            : (useNativeWebGpuSurfaceConsumerBridge
              ? 'native-webgpu-surface-consumer-empty'
              : (useResidentSurfaceBufferHandoffBridge
        ...
        unavailable.renderBridgeReason = unavailable.reason;
        markSphResidentRenderProgress('surface-draw-bridge-empty-render-field', {
          stage: 'surface-draw-bridge',
          surfaceCount: renderFieldSurfaceCount,
      ...
      const nativeSurfaceConsumer = useNativeWebGpuSurfaceConsumerBridge
        ? ensureSphNativeWebGpuSurfaceConsumer({ device })
        : null;
      const rendererCapability = useThreeWebGpuSurfaceBufferBridge
        || useNativeWebGpuSurfaceConsumerBridge
        ? resolveResidentExtensionSurfaceRendererCapability({
          renderer,
          renderBridgeMode: useNativeWebGpuSurfaceConsumerBridge
            ? SPH_NATIVE_WEBGPU_SURFACE_CONSUMER_BRIDGE_MODE
            : SPH_THREE_WEBGPU_SURFACE_BUFFER_BRIDGE_MODE,
          readbackMode: RESIDENT_NO_FULL_READBACK_MODE,
          device,
      ...
      markSphResidentRenderProgress('surface-draw-vertices-started', {
        ...
        effectiveRenderBridgeMode: fallbackThreeWebGpuSurfaceBuffersToCompact
          ? SPH_THREE_COMPACT_VERTEX_BRIDGE_MODE
          : renderBridgeMode,
        ...
        queueCompletionPolicy: waitForSurfaceDrawMetadataQueueCompletion
          ? 'queue.onSubmittedWorkDone-before-direct-consumer'
          : 'same-queue-gpu-handoff-no-cpu-fence'
      });
      surfaceVerticesExecution = await buildSphRenderSurfaceVerticesWebGpu({
        ...
        retainVertexRowsBuffer: !useEffectiveThreeCompactVertexBridge,
        waitForQueueCompletion: useEffectiveThreeCompactVertexBridge,
        deferCleanup: !useEffectiveThreeCompactVertexBridge,
      ...
      if (useEffectiveThreeCompactVertexBridge) {
        markSphResidentRenderProgress('surface-draw-metadata-skipped-three-compact', {
          ...
          reason: fallbackThreeWebGpuSurfaceBuffersToCompact
            ? `Three WebGPU surface buffers fallback to compact geometry: ${rendererCapability?.reason ?? 'same-device buffer geometry unavailable'}`
            : 'Three compact bridge uses surface vertex readback directly'
        });
      ...
      markSphResidentRenderProgress('surface-draw-render-bridge-started', {
        ...
        surfaceCount: surfaceDrawExecution.surfaceCount,
        renderBridgeMode: useEffectiveThreeCompactVertexBridge
          ? SPH_THREE_COMPACT_VERTEX_BRIDGE_MODE
          : (useThreeWebGpuSurfaceBufferBridge
            ? SPH_THREE_WEBGPU_SURFACE_BUFFER_BRIDGE_MODE
            : (useNativeWebGpuSurfaceConsumerBridge
      ...
      const residentDraw = {
        ...
        surfaceDrawBufferMode: useEffectiveThreeCompactVertexBridge
          ...
          : (useThreeWebGpuSurfaceBufferBridge
            ...
            : (useNativeWebGpuSurfaceConsumerBridge
              ? 'native-webgpu-retained-surface-draw-buffers'
              : (useResidentSurfaceBufferHandoffBridge
        ...
        renderBridgeExternalGpuBufferNormalAttribute: Boolean(renderBridge?.externalGpuBufferNormalAttribute),
        renderBridgeExternalGpuBufferNormalAttributeDisabledReason:
          renderBridge?.externalGpuBufferNormalAttributeDisabledReason ?? null,
        renderBridgeExternalGpuBufferIndirect: Boolean(renderBridge?.externalGpuBufferIndirect),
        renderBridgeExternalGpuBufferIndirectRuntimeValidated:
          Boolean(renderBridge?.externalGpuBufferIndirectRuntimeValidated),
        renderBridgeExternalGpuBufferIndirectDisabledReason:
          renderBridge?.externalGpuBufferIndirectDisabledReason ?? null,
      ...
      const surfaceDrawRendererCapability = rendererCapability || {
        status: residentDraw.renderBridgeCapabilityStatus,
        reason: residentDraw.renderBridgeCapabilityReason,
  ...
  async function refreshSphResidentRenderState({
    ...
    try {
      ...
      if (shouldUseResidentRenderRowBridge) {
        ...
        const fieldErrorReason = fieldError instanceof Error ? fieldError.message : String(fieldError);
        const cpuFallbackGeometryAvailable = hasCpuSurfaceGeometryBatches(particleBatches);
        markSphResidentRenderProgress('resident-render-field-fallback-to-render-rows', {
          stage: 'render-field',
          reason: fieldErrorReason,
          cpuFallbackGeometryAvailable
        });
      ...
      if (renderFieldSource === 'resident-gpu-render-field') {
        if (shouldUseResidentRenderRowBridge) {
          ...
          let renderBridgeRetainedPrevious = false;
          const renderBridge = retainPreviousThreeRenderRowBridgeNoFull && !hasRenderRowsReadback
            ? previousResidentRenderBridge
            : (shouldUseWebGpuRenderRowOverlayBridge
          ...
          if (retainPreviousThreeRenderRowBridgeNoFull && renderBridge === previousResidentRenderBridge) {
            renderBridgeRetainedPrevious = true;
            renderBridge.temporalSwapPolicy = SPH_RESIDENT_SURFACE_DRAW_TEMPORAL_SWAP_POLICY;
      ...
      sphResidentRenderState = {
        ...
        renderRowsReadbackRequestedMode: requestedRenderRowsReadbackModeFromCaller,
        renderRowsReadbackEffectiveMode: requestedRenderRowsReadbackMode,
        renderRowsReadbackCoercionReason: renderRowsReadbackModeCoercionReason,
        renderRowsReadbackForcedForThreeBridge: Boolean(
          shouldUseThreeRenderRowPointsBridge
          && requestedRenderRowsReadbackModeFromCaller === RESIDENT_NO_FULL_READBACK_MODE
          && requestedRenderRowsReadbackMode === RESIDENT_FULL_READBACK_MODE
        ),
        ...
        renderFieldReadback: Boolean(renderFieldExecution?.renderFieldReadback),
        renderFieldReadbackMode: renderFieldExecution?.readbackMode ?? null,
        renderFieldReadbackRequestedMode: renderFieldReadbackMode ?? null,
        renderFieldReadbackEffectiveMode: visibleRenderFieldReadbackMode,
        renderFieldReadbackModeCoercionReason,
        ...
        renderFieldSurfaceSummaryMode: requestedRenderFieldSurfaceSummaryMode,
        renderFieldSurfaceSummaryRequestedMode,
        renderFieldSurfaceSummaryModeCoercionReason,
        renderFieldSurfaceSummarySkipped: Boolean(renderFieldSurfaceSummarySkipped),
        renderFieldSurfaceSummarySkipReason: renderFieldSurfaceSummarySkipped?.reason ?? null,
        ...
        surfaceDrawNativeMarchingCubesExtractionSchema:
          sphResidentSurfaceDraw?.nativeMarchingCubesExtractionSchema ?? null,
        surfaceDrawNativeMarchingCubesExtractionAllowed:
          sphResidentSurfaceDraw?.nativeMarchingCubesExtractionAllowed ?? null,
        ...
        surfaceDrawNativeMarchingCubesExtractionErrorStatus:
          sphResidentSurfaceDraw?.nativeMarchingCubesExtractionErrorStatus ?? null,
        surfaceDrawNativeMarchingCubesExtractionErrorStage:
          sphResidentSurfaceDraw?.nativeMarchingCubesExtractionErrorStage ?? null,
        surfaceDrawNativeMarchingCubesExtractionErrorStack:
          sphResidentSurfaceDraw?.nativeMarchingCubesExtractionErrorStack ?? null,
        surfaceDrawNativeMarchingCubesVolumeSchema:
          sphResidentSurfaceDraw?.nativeMarchingCubesVolumeSchema ?? null,
        ...
        surfaceDrawExtensionSurfaceRawExecutionStatus:
          sphResidentSurfaceDraw?.extensionSurfaceRawExecutionStatus ?? null,
        surfaceDrawExtensionSurfaceRawVertexCount:
          sphResidentSurfaceDraw?.extensionSurfaceRawVertexCount ?? null,
        ...
        surfaceDrawVisibleGpuConsumerValidated: Boolean(
          sphResidentSurfaceDraw?.surfaceDrawVisibleGpuConsumerValidated
        ),
        surfaceDrawVisibleGpuConsumerNativeReadbackFallbackValidated: Boolean(
          sphResidentSurfaceDraw?.surfaceDrawVisibleGpuConsumerNativeReadbackFallbackValidated
        ),
        ...
        surfaceDrawRenderBridgeExternalGpuBufferNormalAttributeDisabledReason:
          sphResidentSurfaceDraw?.renderBridgeExternalGpuBufferNormalAttributeDisabledReason ?? null,
        surfaceDrawRenderBridgeExternalGpuBufferIndirect:
          Boolean(sphResidentSurfaceDraw?.renderBridgeExternalGpuBufferIndirect),
        ...
        surfaceDrawRenderBridgeNativeWebGpuSurfaceConsumerEngineIntegration:
          sphResidentSurfaceDraw?.renderBridgeNativeWebGpuSurfaceConsumerEngineIntegration ?? null,
        ...
        surfaceDrawRenderBridgeNativeSurfaceValidationScope:
          sphResidentSurfaceDraw?.renderBridgeNativeSurfaceValidationScope
          ?? sphResidentSurfaceDrawRenderBridge?.nativeSurfaceValidationScope
          ?? null,
        ...
        surfaceDrawRenderBridgeNativeSurfaceOffscreenValidationEligible:
          sphResidentSurfaceDraw?.renderBridgeNativeSurfaceOffscreenValidationEligible
          ?? sphResidentSurfaceDrawRenderBridge?.nativeSurfaceOffscreenValidationEligible
          ?? null,
        ...
        surfaceDrawRenderBridgeNativeSurfaceOffscreenValidationSkippedReason:
          sphResidentSurfaceDraw?.renderBridgeNativeSurfaceOffscreenValidationSkippedReason
          ?? sphResidentSurfaceDrawRenderBridge?.nativeSurfaceOffscreenValidationSkippedReason
          ?? null,
        ...
        surfaceDrawRenderBridgeConfiguredCanvasWidth:
          sphResidentSurfaceDraw?.renderBridgeConfiguredCanvasWidth ?? null,
        surfaceDrawRenderBridgeConfiguredCanvasHeight:
          sphResidentSurfaceDraw?.renderBridgeConfiguredCanvasHeight ?? null,
        surfaceDrawRenderBridgeContextConfigureCount:
          sphResidentSurfaceDraw?.renderBridgeContextConfigureCount ?? null,
        surfaceDrawRenderBridgeLastContextConfigureReason:
          sphResidentSurfaceDraw?.renderBridgeLastContextConfigureReason ?? null,
        surfaceDrawRenderBridgeLastNativeContextReconfigured:
          sphResidentSurfaceDraw?.renderBridgeLastNativeContextReconfigured ?? null,
